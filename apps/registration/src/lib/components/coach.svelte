<script lang="ts">
	...
	let {
		coach = $bindable<Coach>(),
		removable,
		remove,
		primaryCoachIndex = $bindable(),
		index
	} = $props<{
		coach: Coach;
		removable: boolean;
		remove: (index: number) => void;
		primaryCoachIndex: number;
		index: number;
	}>();
</script>
...
<div class="flex flex-row space-x-1.5 align-middle">
	...
	{#if removable}&nbsp;&nbsp;<button
			...
			onclick={() => remove(index)}>Remove coach</button
		>{/if}
</div>
